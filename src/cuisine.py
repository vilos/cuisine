...
def stringify( value ):
    ...
    if   type(value) in (six.text_type, six.binary_type) and len(value) > STRINGIFY_MAXSTRING:
    ...
    if sig != file_md5(location):
        if is_local():
            ...
            if scp:
                ...
                login = shell_safe(hostname) if '@' in hostname else '%s@%s' % (shell_safe(fabric.api.env.user), shell_safe(hostname))
                scp_cmd = 'scp %s %s:%s'% (shell_safe(local_path), login, shell_safe(location))
...
def process_find(name, exact=False):
    ...
    `name`."""
    is_string = isinstance(name, six.string_types)
